...
# if True, downsize image for faster carving
SHOULD_DOWNSIZE = True
# resized image width if SHOULD_DOWNSIZE is True
DOWNSIZE_WIDTH = 500
# large energy value for protective masking
...
def on_mouse(event, x, y, flags, param):
    global im, point1, point2, count, pointsMax
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        for i in range(len(tpPointsChoose)-1):
            print('i', i)
            cv2.line(img2, tpPointsChoose[i],
                     tpPointsChoose[i+1], (0, 0, 255), 5)
...
def ROI_byMouse():
    ...
    cv2.imwrite('.\Mask_imgs\mask.jpg', mask2)
...
def object_removal(im, rmask, mask=None, vis=False, horizontal_removal=False):
    ...
    output, mask = seams_insertion(
        output, num_add, mask, vis, rot=horizontal_removal)
    if horizontal_removal:
        output = rotate_image(output, False)
...
if __name__ == '__main__':
    ...
    ap.add_argument("-rmask", help="Check condition to removal mask")
    ap.add_argument(
    ...
    IM_PATH, MASK_PATH, OUTPUT_NAME, R_MASK_CHECKED = args[
        "im"], args["mask"], args["out"], args["rmask"]
    ...
    #Call mouse event to draw multip line -> define an area called mask ->remove
    if R_MASK_CHECKED:
        while True:
        ...
        if R_MASK_CHECKED:
            rmask = cv2.imread(".\Mask_imgs\mask.jpg", 0) 
        else:
            rmask = None
